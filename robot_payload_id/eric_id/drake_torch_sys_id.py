...
from .containers import take_first
...
from .friction import calc_joint_dry_friction, regularizer_tanh
from .friction_fit import JointFriction, PositiveScalar
from .sys_id_param import (
    ApplyBodyParam,
    ApplyJointParam,
    ExtractBodyParam,
    ExtractJointParam,
...
class DynamicsModel(nn.Module):
    ...
    @classmethod
    def from_plant(cls, plant, bodies, inertial_param_cls=LogCholeskyInertialParameter):
...
class DynamicsModelTrajectoryLoss(nn.Module):
    """
    A dynamics model trajectory loss between the model predicted and measured torques.
    It includes a regularization term based on the entropic inertia divergence from
    ...
    def __init__(
        ...
        model,
        gamma: float,
        torque_loss: Callable[[Iterable, Iterable], float] = nn.functional.mse_loss,
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """_summary_

         Args:
            model (_type_): _description_
            gamma (float): The weight of the regularization term.
            torque_loss (Callable[[Iterable, Iterable], float], optional): The loss
            function.

        Returns:
            Tuple[torch.Tensor, Dict[str, torch.Tensor]]: A tuple of (combined_loss,
            loss_dict).
        """
        super().__init__()
        self.model = model
        ...
        self.gamma = gamma
        self.regularizer = InertialEntropicDivergence(masses, coms, rot_inertias)
        self.torque_loss = torque_loss

    def forward(self, q, v, vd, tau_cmd_target, *, static=False):
        torque_cmd_est = self.model(q, v, vd, static=static)
        torque_loss = self.torque_loss(tau_cmd_target, torque_cmd_est)
        ...
        loss = torque_loss + regularizer_loss
        loss_dict = {
            "tau": torque_loss.detach().item(),
...
def extract_model_param_from_torch(plant, bodies, joints, dyn_model):
    ...
    model_param = extract_model_param_from_plant(plant, context, bodies, joints)
