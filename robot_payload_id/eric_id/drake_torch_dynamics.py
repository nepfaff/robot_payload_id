...
from pydrake.multibody.tree import MultibodyForces_, SpatialInertia_, UnitInertia_
from torch import nn

from .dair_pll_inertia import (
    InertialParameterConverter,
    inertia_matrix_from_vector,
    inertia_vector_from_matrix,
    parallel_axis_theorem,
)
from .drake_torch_autodiff import drake_torch_function
from .multibody_extras import are_frames_welded, get_bodies, get_joints
...
def set_plant_inertial_params(plant, context, bodies, masses, coms, rot_inertias):
...
class LogCholeskyLinAlgInertialParameter(InertialParameter):
    ...
    def __init__(self, masses, coms, rot_inertias):
        super().__init__(masses, coms, rot_inertias)
        pseudo_inertias = inertial_param_to_pseudo_inertia(masses, coms, rot_inertias)
        theta = matrix_to_log_cholesky_vector(pseudo_inertias)
        self.theta = nn.Parameter(theta)

    def forward(self):
        pseudo_inertias = log_cholesky_vector_to_matrix(self.theta)
        masses, coms, rot_inertias = pseudo_inertia_to_inertial_param(pseudo_inertias)
        return masses, coms, rot_inertias
...
class UnitLogCholeskyInertialParameter(InertialParameter):
    ...
    def forward(self):
        ...
        pseudo_inertias = unit_pseudo_inertias * self.masses.reshape((-1, 1, 1))
        _, coms, rot_inertias = pseudo_inertia_to_inertial_param(pseudo_inertias)
        return masses, coms, rot_inertias
...
def inertial_param_to_pseudo_inertia(masses, coms, rot_inertias):
    ...
    N = masses.shape[0]
    pseudo_inertias = torch.zeros((N, 4, 4), device=masses.device, dtype=masses.dtype)
...
class PseudoInertiaCholeskyInertialParameter(InertialParameter):
    ...
    def __init__(self, masses, coms, rot_inertias):
        super().__init__(masses, coms, rot_inertias)
        with torch.no_grad():
            vec = inertial_param_to_pseudo_inertia_cholesky(masses, coms, rot_inertias)
        self.vec = nn.Parameter(vec)

    def forward(self):
        masses, coms, rot_inertias = pseudo_inertia_cholesky_to_inertial_param(self.vec)
        return masses, coms, rot_inertias
...
def inertial_param_to_log_cholesky_com(masses, coms, rot_inertias):
    ...
    """
    rot_inertias_cm = parallel_axis_theorem(rot_inertias, masses, coms, Ba_is_Bcm=False)
...
def log_cholesky_com_to_inertial_param(x):
    ...
    rot_inertias = parallel_axis_theorem(rot_inertias_cm, masses, coms, Ba_is_Bcm=True)
    return masses, coms, rot_inertias
...
class LogCholeskyComInertialParameter(InertialParameter):
    ...
    def __init__(self, masses, coms, rot_inertias):
        super().__init__(masses, coms, rot_inertias)
        with torch.no_grad():
            vec = inertial_param_to_log_cholesky_com(masses, coms, rot_inertias)
        self.vec = nn.Parameter(vec)
...
def inertial_entropic_divergence(
    ...
    pseudo_inertias_init_inv = pseudo_inertias_init_inv.to(pseudo_inertias.dtype)
...
class InertialEntropicDivergence(nn.Module):
    """
    Entropic Divergence distance between two sets of inertial parameters from "Geometric
    Robot Dynamic Identification: A Convex Programming Approach"
    (https://ieeexplore.ieee.org/document/8922724).
    """
    ...
    def __init__(self, masses, coms, rot_inertias):
        ...
        with torch.no_grad():
            ...
            self.pseudo_inertias_init_inv = torch.linalg.inv(pseudo_inertias_init)
            self.logdet_pseudo_inertias_init = torch.logdet(pseudo_inertias_init)

    def forward(self, masses, coms, rot_inertias):
        psuedo_inertias = inertial_param_to_pseudo_inertia(masses, coms, rot_inertias)
...
def plant_inverse_dynamics(
    ...
    set_plant_inertial_params(plant, context, bodies, masses, coms, rot_inertias)
