...
def optimize_traj_snopt(
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray]: The optimized trajectory parameters `a` and `b`.
            NOTE: The final parameters are returned regardless of whether the
    ...
    joint_data = compute_autodiff_joint_data_from_simple_sinusoidal_traj_params(
        num_timesteps=1000, timestep=1.0, a=a, b=b, omega=0.5
    )
    ...
    if res.is_success():
        ...
        logging.info(f"a: {res.GetSolution(a)}")
        logging.info(f"b: {res.GetSolution(b)}")
    else:
...
def optimize_traj_black_box(
    ...
    """Optimizes the trajectory parameters using black box optimization.
    ...
    # Solve
    optimizer = ng.optimizers.NGOpt(parametrization=len(a) + len(b), budget=budget)
    recommendation = optimizer.minimize(cost_function_to_cost[cost_function])
    logging.info(f"Final loss: {recommendation.loss}")
    logging.info(f"Final param values: {recommendation.value}")

    return recommendation.value[: len(a)], recommendation.value[len(a) :]
